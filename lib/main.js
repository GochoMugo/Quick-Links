...
if (QL_var.version != QL_var.storage.version) {
    ...
    function majorBump(version) {
        if (!QL_var.storage.version) return false;

        var old_version = QL_var.storage.version.substr(0, QL_var.storage.version.lastIndexOf('.'));
        var new_version = QL_var.version.substr(0, QL_var.version.lastIndexOf('.'));
